                ...
                for (int sliceIdx = 0; sliceIdx < numSlices; sliceIdx++)
                {
                    ...
                    if (sliceTex != null)
                    {
                        if (!MBVersion.IsTextureReadable(sliceTex))
                        {
                            ...
                            {
                                Debug.LogError("Source texture must be readable: " + sliceTex);
                                return false;
                            }
                        }

                        bool isAsset = true;
                        if (textureEditorMethods != null) isAsset = textureEditorMethods.IsAnAsset(sliceTex);
                        if (logLevel >= MB2_LogLevel.trace) Debug.Log("Considering format of texture: " + sliceTex + " format:" + sliceTex.format);
                        if (Application.isPlaying)
                        {
                            if (sliceTex.width != targetWidth || sliceTex.height != targetHeight || sliceTex.format != format)
                            {
                                Debug.LogError("If creating Texture Arrays at runtime then source textures must be the same size as the texture array and in the same format as the texture array." +
                                     "Texture " + sliceTex.name + " is not in the correct format or does not have the correct size. (" + sliceTex.width + ", " + sliceTex.height + ", " + sliceTex.format);
                                return false;
                            }
                        }
                        else
                            ...
                            float maxTimePerFrame = .01f)
        {
            ...
            try
            {
                ...
                for (int texFormatSetIdx = 0; texFormatSetIdx < textureArrayOutputFormats.Length; texFormatSetIdx++)
                {
                    ...
                    if (editorMethods != null) editorMethods.Clear();
            ...
            {
                if (editorMethods != null) editorMethods.RestoreReadFlagsAndFormats(progressInfo);
                combiner._destroyAllTemporaryTextures();
                for (int i = 0; i < temporaryTextureAssets.Count; i++)
                {
                    if (editorMethods != null) editorMethods.DestroyAsset(temporaryTextureAssets[i]);
                }
