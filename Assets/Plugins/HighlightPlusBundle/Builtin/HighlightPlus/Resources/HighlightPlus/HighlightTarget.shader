...
Properties {
    ...
    float GetRawDepth(float2 uv) {
        return UNITY_SAMPLE_DEPTH(UNITY_SAMPLE_SCREENSPACE_TEXTURE(_CameraDepthTexture, uv));
    }
    ...
    float3 GetWorldNormal(float2 uv) {
        float2 ipos = uv * _CameraDepthTexture_TexelSize.zw;
        half3 viewNormal = ViewNormalAtPixelPosition(ipos);
        ...
        return normalWS;
            ...
            half4 frag(v2f i) : SV_Target
            {
                ...
                float depth = GetRawDepth(screenUV);
                float3 decalPos;
                ...
                // check normal
                float3 norm = GetWorldNormal(screenUV);
                float slope = dot(norm, GROUND_NORMAL);
                clip(slope - 0.01);
