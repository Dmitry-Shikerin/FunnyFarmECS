            ...
            if (seeThroughOccluderCheckIndividualObjects) {
                for (int r = 0; r < rms.Length; r++) {
                    if (rms[r].renderer != null) {
                        ...
                        if (Physics.BoxCast(pos, extents, (camPos - pos).normalized, out RaycastHit hitInfo, Quaternion.identity, maxDistance, seeThroughOccluderMask)) {
                            occlusionData.cachedOccluderCollider = hitInfo.collider;
                            occlusionData.lastOcclusionTestResult = true;
                            return true;
                        }
                        // Need to check if the occluder is overlapping the object
                        if (Physics.OverlapBoxNonAlloc(pos, extents, colliders, Quaternion.identity, seeThroughOccluderMask) > 0) {
                            occlusionData.cachedOccluderCollider = colliders[0];
                            occlusionData.lastOcclusionTestResult = true;
                            return true;
                        }
                    }
