...
namespace TeoGames.Mesh_Combiner.Scripts.Extension {
	...
	public static class MeshBonesExtension {
		private static readonly Dictionary<Transform, (int index, Matrix4x4 pose)> _BONES =
			new Dictionary<Transform, (int index, Matrix4x4 pose)>();

		private static readonly Dictionary<int, int> _MAP_INDEXES = new Dictionary<int, int>();

		private static readonly Dictionary<Mesh, (Mesh mesh, int[] bones)> _OPTIMIZED_MESHES =
		...
		private static void ResetCache() {
			_BONES.Clear();
			_MAP_INDEXES.Clear();
			_OPTIMIZED_MESHES.Clear();
		}
#endif

		public static (Mesh, Transform[]) Optimize(this Mesh mesh, Transform[] bones, bool hasBones) {
			if (_OPTIMIZED_MESHES.TryGetValue(mesh, out var res) && res.mesh) {
			...
			var (newBones, keys) = newMesh.RemoveUnusedBones(bones, hasBones);

			res = (newMesh, keys);
			_OPTIMIZED_MESHES[mesh] = res;
		...
		private static (Transform[], int[]) RemoveUnusedBones(this Mesh mesh, Transform[] bones, bool hasBones) {
			var origBindPoses = mesh.bindposes;
			var bonesLength = bones.Length;
			if (!hasBones || origBindPoses.Length == 0 || bonesLength == 0) {
				return (bones, Enumerable.Range(0, bones.Length).ToArray());
			}
			...
			for (var i = 0; i < weightsCount; i++) {
				...
				if (orig.weight0 > 0 && _MAP_INDEXES.TryGetValue(orig.boneIndex0, out var index)) orig.boneIndex0 = index;
				if (orig.weight1 > 0 && _MAP_INDEXES.TryGetValue(orig.boneIndex1, out index)) orig.boneIndex1 = index;
				if (orig.weight2 > 0 && _MAP_INDEXES.TryGetValue(orig.boneIndex2, out index)) orig.boneIndex2 = index;
				if (orig.weight3 > 0 && _MAP_INDEXES.TryGetValue(orig.boneIndex3, out index)) orig.boneIndex3 = index;
			...
			return (newBones.ToArray(), _MAP_INDEXES.Keys.ToArray());
		}
		...
		public static async Task<Transform[]> MergeBones(this Mesh mesh, Transform[] allBones, int maxExecTime) {
			...
			for (var i = 0; i < allBonesCount; i++) {
				...
				if (_BONES.TryGetValue(bone, out var pair)) {
					if (i != pair.index) {
						if (changedAt > i) changedAt = i;
						_MAP_INDEXES.Add(i, pair.index);
					}
				} else {
					if (i != id) _MAP_INDEXES.Add(i, id);
					_BONES.Add(bone, (id, origBindPoses[i]));
					id++;
				}
			}

			if (_BONES.Count == allBonesCount) return allBones;
			...
			for (var c = 0; c < weightsCount; c += 10000) {
				...
				for (var i = c; i < stop; i++) {
					...
					orig.boneIndex = _MAP_INDEXES[boneIndex];
					weights[i] = orig;
				}
			...
			var j = 0;
			foreach (var (_, pose) in _BONES.Values) poses[j++] = pose;

			mesh.bindposes = poses;
			mesh.SetBoneWeights(mesh.GetBonesPerVertex(), weights);
			weights.Dispose();

			return _BONES.Keys.ToArray();
		}
	}
}