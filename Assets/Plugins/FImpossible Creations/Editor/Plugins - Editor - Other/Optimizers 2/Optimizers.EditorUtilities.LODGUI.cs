...
namespace FIMSpace.FOptimizing
{
    internal static class Optimizers_LODGUI
    {
        ...
        public static void MoveCameraToDistance( Optimizer_Base optimizer, float distance )
        {
            SceneView view = SceneView.lastActiveSceneView;
            if( view == null ) return;

            Vector3 focPoint = optimizer.transform.position;
            focPoint += optimizer.transform.TransformVector( optimizer.DetectionOffset );

            if( optimizer.OptimizingMethod == EOptimizingMethod.Effective || optimizer.OptimizingMethod == EOptimizingMethod.Static )
                distance += optimizer.GetDetectionRadiusRaw() * Mathf.Abs( optimizer.transform.lossyScale.x );
                    ...
                    if( opt )
                    {
                        ...
                        else
                        {
                            ...
                            if( opt.OptimizingMethod == EOptimizingMethod.Effective || opt.OptimizingMethod == EOptimizingMethod.Static )
                                distance += opt.GetDetectionRadiusRaw() * Mathf.Abs( opt.transform.lossyScale.x);
