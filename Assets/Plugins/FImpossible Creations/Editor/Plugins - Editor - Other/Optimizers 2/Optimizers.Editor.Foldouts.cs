    ...
    public partial class Optimizer_BaseEditor
    {
        ...
        void Fold_DrawDOTS()
        {
            ...
            if (drawDOTS && Get.UseDOTS)
            {
                ...
                if (OptimizersSetup.Settings.UseProgressiveCulling == false)
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.HelpBox("You need to enable 'Progressive Culling' in optimizers setup to make it work!", MessageType.Info);
                    if (GUILayout.Button("Switch", GUILayout.ExpandHeight(true))) OptimizersSetup.Settings.UseProgressiveCulling = true;
                    if (GUILayout.Button("Setup", GUILayout.ExpandHeight(true))) UnityEditor.Selection.activeObject = OptimizersSetup.Settings;
                ...
                if (Get.DOTSObstacleType == EDOTSObstacle.SeeThrough) if (!OptimizersManager.Settings.SupportNotObstacles) EditorGUILayout.HelpBox("'Not obstacles' are disabled in Optimizers Manager!", MessageType.None);
                EditorGUILayout.EndHorizontal();
