...
namespace FIMSpace.FOptimizing
{
    public abstract partial class Optimizer_Base
    {
        ...
        public void TryAutoComputeDetectionShape( float scaleUp = 1f )
        {
            ...
            DetectionRadius /= Mathf.Abs( transform.lossyScale.x );
            DetectionRadius *= scaleUp;
            DetectionBounds.x /= Mathf.Abs( transform.lossyScale.x );
            DetectionBounds.y /= Mathf.Abs( transform.lossyScale.y );
            DetectionBounds.z /= Mathf.Abs( transform.lossyScale.z );
        ...
        public static Vector3[] MeasureBiggest( Transform t, bool withLodGroups = true, float limitTo = 0, Type checkOnly = null )
        {
            ...
            if( canParticle )
            {
                ...
                if( ps )
                {
                    ...
                    if( psr )
                    {
                        size = Vector3.Scale( psr.bounds.extents, V3Abs( t.lossyScale ) ).magnitude * 2f;
                        if( size > biggest )
                        {
                            biggest = size;
                            if( size > biggestBounds ) biggestBounds = size;
                        ...
                        {
                            biggestBoundsV = Vector3.Scale( psr.bounds.extents, V3Abs( t.lossyScale ) );
            ...
            if( withLodGroups )
            {
                ...
                if( lod )
                {
                    ...
                    size = 0.3f * lod.size * div * Mathf.Abs( lod.transform.lossyScale.x ) / cullPerc;
        ...
        protected static Vector3 V3Abs( Vector3 v )
        {
