    ...
    public partial class EssentialOptimizer
    {
        ...
        public override void AssignComponentsToOptimizeFrom(Component target, bool includeAdvanced = false)
        {
            ...
            for (int i = 0; i < allOnTarget.Length; i++)
            {
                ...
                if (includeAdvanced)
                    type = EssentialLODsController.GetEssentialTypeAll(allOnTarget[i]);
                else
        ...
        public void AddToOptimizeIfCan(Component target, List<Optimizer_Base> childOptimizers)
        {
            ...
            if (_editor_DragAndDropOptim == false)
                if (OptimizationTypes)
                {
                    var type = EssentialLODsController.GetEssentialTypeAll(target);
                    if (OptimizationTypes.IsTypeAllowed(type) == false) return;
                }
