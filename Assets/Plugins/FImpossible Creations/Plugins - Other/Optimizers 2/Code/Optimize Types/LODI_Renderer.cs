...
namespace FIMSpace.FOptimizing
{
    ...
    public sealed class LODI_Renderer : ILODInstance
    {
        ...
        Material[] allocatedMaterials = null;
        public void ApplySettingsToTheComponent(Component component, ILODInstance initialSettingsRef)
        {
            ...
            if (DrawingVersion == 2)
            {
                if (!string.IsNullOrEmpty(shaderParam))
                {
                    if (allocatedMaterials == null) allocatedMaterials = comp.materials;

                    if (allocatedMaterials.Length > 0)
                    {
                        foreach (var item in allocatedMaterials)
                        {
            ...
            if (Disable) comp.enabled = false; else if (!comp.enabled) comp.enabled = true;
        }
