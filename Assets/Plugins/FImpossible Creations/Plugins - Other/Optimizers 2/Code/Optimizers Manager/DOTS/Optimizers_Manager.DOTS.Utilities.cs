...
namespace FIMSpace.FOptimizing
{
    public partial class OptimizersManager
    {
        ...
        private void DOTS_RefreshRaycastFrustum()
        {
            ...
            int dirsCount = Mathf.RoundToInt(scaleRatio / Settings.ProgressiveResponseQuality) * Settings.ProgressiveResponseQuality;
        ...
        {
            if (Settings.ProgAutoDelay)
            {
                float mul = Mathf.Lerp(1f, 0.65f, UpdateBoost);
                float mul2 = Mathf.Lerp(1f, 0.75f, UpdateBoost);

                if (Settings.ProgressiveResponseQuality < 1000)
                    Settings.ProgCullDelay = Mathf.Lerp(3.5f * mul, 2f * mul, Mathf.InverseLerp(100, 1000, Settings.ProgressiveResponseQuality));
                else
                    Settings.ProgCullDelay = Mathf.Lerp(2f * mul, 0.5f * mul2, Mathf.InverseLerp(1000, 6000, Settings.ProgressiveResponseQuality));
            }
        }
