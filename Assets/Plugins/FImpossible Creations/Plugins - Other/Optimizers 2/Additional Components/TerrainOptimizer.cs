...
namespace FIMSpace.FOptimizing
{
    ...
    public class TerrainOptimizer : ScriptableOptimizer, UnityEngine.EventSystems.IDropHandler, IFHierarchyIcon
    {
        ...
        [Tooltip("Use it to enlarge culling spheres volume if you want to use less spheres but ensure to cover all terrain surface.")]
        public float ExtraRadiusSize = 0f;
        ...
        protected BoundingSphere[] GetBoundingSpheres()
        {
            ...
            float spheresRadius = DetectionRadius * (1f + ExtraRadiusSize);
            ...
            for (int x = 0; x <= xSteps; x++)
                for (int z = 0; z <= zSteps; z++)
                {
                    ...
                    pos.y = Terrain.SampleHeight(pos) + transform.position.y;

                    Color sColor = Color.HSVToRGB(i / max, 0.9f, 0.8f); sColor.a = GizmosAlpha;
                    Gizmos.color = sColor;

                    if (z != zSteps && x != xSteps)
                        spheres.Add(new BoundingSphere(pos, spheresRadius));
