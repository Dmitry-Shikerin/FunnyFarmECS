...
namespace MegaFiers
{
	public class MegaGUI : MonoBehaviour
	{
		...
		bool[]				showmod;
		bool[]				showgmod;
		MegaModifyObject	context		= null;
		MegaModifyObject	gcontext	= null;
		...
		void Start()
		{
			if ( source != null )
			{
				context = (MegaModifyObject)source.GetComponent<MegaModifyObject>();
				...
				gcontext = (MegaModifyObject)ground.GetComponent<MegaModifyObject>();
		...
		public Color bcol = new Color(245, 177, 17);
		...
		void ShowGUI(MegaModifier mod)
		{
			switch ( mod.ModName() )
			{
				...
				case "Hump":
					...
					hmod.amount = ProSlider("Amount", hmod.amount, -2.0f, 2.0f, sz);
		...
		void OnGUI()
		{
			if ( ShowGui && mods != null )
			{
				...
				windowRect = GUILayout.Window(0, windowRect, DoWindow, source.name + " - Modifiers", GUILayout.Width(100));
			}
		}
	}
}