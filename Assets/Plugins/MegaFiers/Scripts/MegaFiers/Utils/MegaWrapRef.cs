...
namespace MegaFiers
{
	...
	public class MegaWrapRef : MonoBehaviour
	{
		...
		{
			int bcount = mesh1.blendShapeCount;
		...
		void DoUpdate()
		{
			if ( source == null || WrapEnabled == false || target == null || source.bindverts == null )
				return;
				...
				for ( int i = 0; i < source.bindverts.Length; i++ )
				{
					if ( source.bindverts[i].verts.Count > 0 )
					{
						...
						for ( int j = 0; j < source.bindverts[i].verts.Count; j++ )
						{
							...
							Vector3 p0 = target.jsverts[bi.i0];
							Vector3 p1 = target.jsverts[bi.i1];
							Vector3 p2 = target.jsverts[bi.i2];
