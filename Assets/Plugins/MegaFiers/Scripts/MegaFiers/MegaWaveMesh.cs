...
namespace MegaFiers
{
	...
	public class MegaWaveMesh : MonoBehaviour
	{
		...
		static public float WaveFunc(float radius, float t, float amp, float waveLen, float phase)
		{
			float ang = Mathf.PI * 2.0f * (radius / waveLen + phase);
			return amp * Mathf.Sin(ang);
		}

		static public float WaveFunc1(float radius, float t, float amp, float waveLen, float phase)
		{
		...
		void UpdateSurface()
		{
			...
			dist1 = (wave1 / 10.0f) * 4.0f * 5.0f;

			if ( dist1 == 0.0f )
				dist1 = 1.0f;

			dist2 = (wave2 / 10.0f) * 4.0f * 5.0f;
