	...
	public class MegaDynamicRipple : MegaModifier
	{
		...
		public override void Modify(MegaModifyObject mc)
		{
			for ( int i = 0; i < jverts.Length; i++ )
			{
				...
				p[vertcomponent] += currentBuffer[vertIndex] * scale;
		...
		void Setup()
		{
			...
			{
				float column = ((verts[i][xc] - bbox.min[xc]) / size[xc]);
				float row = ((verts[i][yc] - bbox.min[yc]) / size[yc]);
				...
				float position = (ri * (cols)) + ci;
				vertexIndices[i] = (int)position;
			}
		}
		...
		public void wakeAtPointAdd1(float x, float y, float force)
		{
			...
			for ( int j = yi; j < yi + 2; j++ )
			{
				for ( int i = xi; i < xi + 2; i++ )
				{
					...
					{
						if ( dists[index] < 1.0f )
							input[i + (j * cols)] = force * (dists[index] / tdist);
					}
					index++;
				}
			}
		...
		void Update()
		{
			...
			int drops = (int)(time * DropsPerSec);
		...
		void BuildMesh()
		{
			...
			if ( currentBuffer != null && currentBuffer.Length == rows * cols )
			{
