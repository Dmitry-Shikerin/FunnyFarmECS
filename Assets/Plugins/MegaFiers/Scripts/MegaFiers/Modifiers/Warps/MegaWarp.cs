...
namespace MegaFiers
{
	...
	public class MegaWarp : MonoBehaviour
	{
		...
		private void Update()
		{
			if ( transform.position != lastpos || transform.eulerAngles != lastrot || transform.localScale != lastscl )
			{
				lastpos = transform.position;
				lastrot = transform.eulerAngles;
				lastscl = transform.localScale;
				changed = true;
			}
		}
