...
namespace MegaFiers
{
	...
	public class MegaModifier : MonoBehaviour
	{
		...
		public Vector3[]				checkCorners		= new Vector3[8];	// used for auto update and deform checks
		public Vector3[]				corners				= new Vector3[8];	// used by gizmo drawing
		protected Vector3[]				lastCorners			= new Vector3[8];
		...
		public string					Label				= "";
		public int						MaxLOD				= 0;
		public bool						showModParams		= false;
		public bool						showParams			= true;
		public bool						alwaysChanging		= false;
		...
		public virtual void				ModStart(MegaModifyObject ms)		{ }
		...
		public virtual bool AnyDeform(float epsilon = 0.0001f)
		{
			if ( ModEnabled )
			{
				for ( int i = 0; i < checkCorners.Length; i++ )
		...
		{
			lastCorners[0] = checkCorners[0] + Vector3.up;
			lastCorners[1] = checkCorners[1] + Vector3.up;
			lastCorners[7] = checkCorners[7] + Vector3.up;
		}
		...
		public void SetModMesh(Mesh ms)
		{
			if ( ms != null )
			{
				...
				corners[6] = new Vector3(b.max.x, b.max.y, b.max.z);
				corners[7] = new Vector3(b.max.x, b.min.y, b.max.z);

				for ( int i = 0; i < 8; i++ )
					checkCorners[i] = new Vector3(Mathf.Lerp(b.min.x, b.max.x, Random.value), Mathf.Lerp(b.min.y, b.max.y, Random.value), Mathf.Lerp(b.min.z, b.max.z, Random.value));

				//checkCorners[0] = new Vector3(b.min.x * 0.75f, b.min.y, b.min.z);
				//checkCorners[1] = new Vector3(b.min.x, b.max.y * 0.75f, b.min.z);
				//checkCorners[2] = new Vector3(b.max.x, b.max.y, b.min.z * 0.75f);
				//checkCorners[3] = new Vector3(b.max.x * 0.75f, b.min.y * 0.75f, b.min.z * 0.75f);

				//checkCorners[4] = new Vector3(b.min.x * 0.9f, b.min.y, b.max.z);
				//checkCorners[5] = new Vector3(b.min.x, b.max.y, b.max.z);
				//checkCorners[6] = new Vector3(b.max.x, b.max.y, b.max.z);
				//checkCorners[7] = new Vector3(b.max.x, b.min.y, b.max.z);
		...
		public virtual void Modify(MegaModifyObject mc)
		{
			if ( verts != null )
			{
		...
		public virtual void ModifyNoJobs(MegaModifyObject mc)
		{
			if ( verts != null )
			{
				for ( int i = 0; i < jverts.Length; i++ )
		...
		public virtual void ModifyWeighted(MegaModifyObject mc)
		{
			for ( int i = 0; i < jverts.Length; i++ )
			{
				Vector3 p = jverts[i];
