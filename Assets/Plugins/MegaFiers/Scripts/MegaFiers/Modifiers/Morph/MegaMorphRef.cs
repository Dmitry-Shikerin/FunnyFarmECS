	...
	public class MegaMorphRef : MegaMorphBase
	{
		...
		public override bool ModLateUpdate(MegaModContext mc)
		{
			if ( source )
			{
				...
				if ( dif == null )
				{
					dif = new Vector3[mc.mod.jverts.Length];
				}
			}
			return Prepare(mc);
		}
						...
						else
						{
...

							{
								for ( int pointnum = 0; pointnum < source.oPoints.Length; pointnum++ )
								{
									...
									dif[pointnum].x += progession.x - vert.x;
									dif[pointnum].y += progession.y - vert.y;
									dif[pointnum].z += progession.z - vert.z;
								}
							}
						}
					}
