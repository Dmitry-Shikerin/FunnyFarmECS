	...
	public class MegaShapeEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if ( GUILayout.Button("Add Knot") )
			{
				...
				CursorTangent = shape.splines[curve].Interpolate(per + 0.01f, true, ref CursorKnot);
				CursorPos = shape.splines[curve].Interpolate(per, true, ref CursorKnot);
				...
				if ( shape.smoothonaddknot )
					shape.AutoCurve(shape.splines[shape.selcurve]);
		...
		Vector3 CircleCap(int id, Vector3 pos, Quaternion rot, float size)
		{
			...
			return Handles.FreeMoveHandle(cid, pos, size, Vector3.zero, Handles.CircleHandleCap);
#else
		...
		{
#if UNITY_2022_1_OR_NEWER
			return Handles.FreeMoveHandle(cid, pos, size, Vector3.zero, Handles.SphereHandleCap);
#else
					...
					if ( hn[0] == 'a' )
					{
						...
						else
						{
							if ( ak == shape.splines[shape.selcurve].knots.Count - 1 )
								mp = shape.InterpCurve3D(shape.selcurve, 1.02f, true);
							else
			...
			for ( int s = 0; s < shape.splines.Count; s++ )
			{
				for ( int p = 0; p < shape.splines[s].knots.Count; p++ )
				{
					...
					if ( shape.drawKnots && s == shape.selcurve )
					{
						...
						{
							if ( shape.usesnap )
								newp = PosHandlesSnap(shape, pm, fwd);
							else
								newp = PosHandles(shape, pm, fwd);
					...
					if ( s == shape.selcurve )
					{
						...
						else
						{
							...
							else
							{
								...
								Vector3 mp = shape.InterpCurve3D(s, 1.02f, true);
								SetControlName("a" + p.ToString());
								CircleCap(0, mp, Quaternion.identity, shape.KnotSize * 0.01f);
							}
						}
		...
		static void RenderGizmo(MegaShape shape, GizmoType gizmoType)
		{
			if ( (gizmoType & GizmoType.Active) != 0 && Selection.activeObject == shape.gameObject )
			{
				...
				if ( shape.splines[shape.selcurve].knots.Count > 1 )
				{
					...
					if ( MegaShapeEditor.editor != null && editmode )
					{
		...
		void LoadSVG(float scale)
		{
			...
			svg.importData(text, ms, scale, opt, startspline);
			ms.imported = true;
		}
		...
		void LoadSXL(float scale)
		{
			...
			sxl.importData(text, ms, scale, opt, startspline);
			ms.imported = true;
		}
