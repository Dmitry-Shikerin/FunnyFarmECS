...
{
    /// <summary>
    /// Specialized Reactor class that allows you to animate any public Vector2 field or property of any object via Reactor.
    /// </summary>
    [AddComponentMenu("Reactor/Animators/Vector2 Animator")]
    public class Vector2Animator : ReflectedValueAnimator
    {
        #if UNITY_EDITOR
        [UnityEditor.MenuItem("GameObject/Reactor/Animators/Vector2 Animator", false, 6)]
        private static void CreateComponent(UnityEditor.MenuCommand menuCommand)
        {
            GameObjectUtils.AddToScene<Vector2Animator>(false, true);
        }
        #endif

        /// <summary> Vector2 value target accessed via reflection </summary>
        public ReflectedVector2 ValueTarget;

        /// <summary> Check if the value target is set up correctly </summary>
        public bool isValid => ValueTarget.IsValid();

        #pragma warning disable CS0109
        [SerializeField] private Vector2Animation Animation;
        /// <summary> Vector2 Animation </summary>
        public new Vector2Animation animation => Animation ?? (Animation = new Vector2Animation(ValueTarget));
        #pragma warning restore CS0109

        #if UNITY_EDITOR
        private void Reset()
        {
            Animation ??= new Vector2Animation(ValueTarget);
            ResetAnimation();
        }
        #endif
        ...
        /// <summary> Set the value target </summary>
        /// <param name="reflectedValue"> Reflected value target </param>
        public void SetTarget(ReflectedVector2 reflectedValue) =>
