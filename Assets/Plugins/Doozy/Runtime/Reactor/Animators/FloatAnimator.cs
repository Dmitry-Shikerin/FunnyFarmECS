...
{
    /// <summary>
    /// Specialized Reactor class that allows you to animate any public float field or property of any object via Reactor.
    /// </summary>
    [AddComponentMenu("Reactor/Animators/Float Animator")]
    public class FloatAnimator : ReflectedValueAnimator
    {
        #if UNITY_EDITOR
        [UnityEditor.MenuItem("GameObject/Reactor/Animators/Float Animator", false, 6)]
        private static void CreateComponent(UnityEditor.MenuCommand menuCommand)
        {
            GameObjectUtils.AddToScene<FloatAnimator>(false, true);
        }
        #endif
        ...
        [SerializeField] private FloatAnimation Animation;
        /// <summary> Float Animation </summary>
        public new FloatAnimation animation => Animation ?? (Animation = new FloatAnimation(ValueTarget));
        #pragma warning restore CS0109

        #if UNITY_EDITOR
        private void Reset()
        {
            Animation ??= new FloatAnimation(ValueTarget);
            ResetAnimation();
        }
        #endif
