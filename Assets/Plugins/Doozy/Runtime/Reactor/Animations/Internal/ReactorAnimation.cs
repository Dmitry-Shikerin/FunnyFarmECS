...
namespace Doozy.Runtime.Reactor.Animations
{
    ...
    public abstract class ReactorAnimation
    {
        ...
        /// <summary> Check if the animation has a target </summary>
        ...
        /// <summary> Internal variable used to prevent Coroutiner trying indefinitely to start the animation, when a target is not set. </summary>
        private int m_InfiniteLoopCount = 0;
        /// <summary> The maximum number of times Coroutiner will try to start the animation, when a target is not set. </summary>
        ...
        public virtual void SetProgressAt(float targetProgress)
        {
            if (!hasTarget)
            {
                if (m_InfiniteLoopCount > MAX_INFINITE_LOOP_COUNT) return;
                m_InfiniteLoopCount++;
                Coroutiner.ExecuteAtEndOfFrame(() => SetProgressAt(targetProgress));
                return;
            }
        ...
        public virtual void PlayToProgress(float toProgress)
        {
            if (!hasTarget)
            {
                if (m_InfiniteLoopCount > MAX_INFINITE_LOOP_COUNT) return;
                m_InfiniteLoopCount++;
                Coroutiner.ExecuteAtEndOfFrame(() => PlayToProgress(toProgress));
                return;
            }
            m_InfiniteLoopCount = 0;
            StopAllReactionsOnTarget();
        ...
        public virtual void PlayFromProgress(float fromProgress)
        {
            if (!hasTarget)
            {
                if (m_InfiniteLoopCount > MAX_INFINITE_LOOP_COUNT) return;
                m_InfiniteLoopCount++;
                Coroutiner.ExecuteAtEndOfFrame(() => PlayFromProgress(fromProgress));
                return;
            }
            m_InfiniteLoopCount = 0;
            StopAllReactionsOnTarget();
        ...
        public virtual void PlayFromToProgress(float fromProgress, float toProgress)
        {
            if (!hasTarget)
            {
                if (m_InfiniteLoopCount > MAX_INFINITE_LOOP_COUNT) return;
                m_InfiniteLoopCount++;
                Coroutiner.ExecuteAtEndOfFrame(() => PlayFromToProgress(fromProgress, toProgress));
                return;
            }
            StopAllReactionsOnTarget();
            UpdateValues();
