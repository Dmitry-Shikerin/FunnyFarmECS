...
{
    [Serializable]
    public class StreamIdDatabase : SingletonEditorScriptableObject<StreamIdDatabase>, IUpdateCallback
    {
        [SerializeField] private StreamIdDataGroup Database;
        public StreamIdDataGroup database => Database ??= new StreamIdDataGroup();
        ...
        public StreamIdDatabase() => Database = new StreamIdDataGroup();


        public (bool, string) CanImportRoamingDatabases(List<StreamIdRoamingDatabase> roamingDatabases)
        {
            roamingDatabases ??= new List<StreamIdRoamingDatabase>();           //avoid null
        ...
        {
            var roamingDatabases = databases.Cast<StreamIdRoamingDatabase>().ToList();
            bool canImport;
            string message;
            ...
            foreach (StreamIdRoamingDatabase roamingDatabase in roamingDatabases)
            {
                counter++;
        ...
        {
            string[] guids = AssetDatabase.FindAssets($"t:{nameof(StreamIdRoamingDatabase)}");
            if (guids == null || guids.Length == 0)
            {
                EditorUtility.DisplayDialog("Import Databases", $"No {nameof(StreamIdRoamingDatabase)} databases were found", "Ok");
                return false;
            }
            ...
            {
                counter++;
                StreamIdRoamingDatabase asset = AssetDatabase.LoadAssetAtPath<StreamIdRoamingDatabase>(AssetDatabase.GUIDToAssetPath(guid));
        ...
        public bool ExportRoamingDatabase(string databaseName, List<string> selectedCategories)
        {
            ...
            StreamIdRoamingDatabase roamingDatabase = CreateInstance<StreamIdRoamingDatabase>();
            roamingDatabase.databaseName = databaseName;
            roamingDatabase.database.ClearDatabase();
            string savePath = EditorUtility.SaveFilePanelInProject("Export Roaming Database", $"{databaseName}_{nameof(StreamIdRoamingDatabase)}", "asset", "Save to");
