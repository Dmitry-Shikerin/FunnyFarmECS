...
{
    [CustomEditor(typeof(EditorDataLayoutGroup))]
    public class EditorDataLayoutGroupEditor : UnityEditor.Editor, ISearchable
    {
        private EditorDataLayoutGroup castedTarget => (EditorDataLayoutGroup)target;
        ...
        private static string ListViewTitle(string groupName) =>
            $"{(groupName.IsNullOrEmpty() ? "Unnamed" : groupName)} Layout Group";
        ...
        private void Save() =>
            EditorDataLayoutDatabase.instance.RefreshDatabaseItem(castedTarget, true, true, true);

        private void LoadUxmlReferencesFromFolder()
        {
            castedTarget.LoadUxmlReferencesFromFolder();
        ...
        {
            arrayProperty = serializedObject.FindProperty("Layouts");
            ...
            fluidListView.SetListDescription("List of .uxml layouts");
        ...
        public void UpdateSearchResults()
        {
            ...
            for (int i = 0; i < arrayProperty.arraySize; i++)
            {
                ...
                Object objectReferenceValue = arrayElementAtIndex.FindPropertyRelative("UxmlReference").objectReferenceValue;
