...
{
    [CustomEditor(typeof(EditorDataTextureGroup))]
    public class EditorDataTextureGroupEditor : UnityEditor.Editor, ISearchable
    {
        ...
        public FluidField prefixComponentField { get; private set; }
        ...
        private static string ListViewTitle(string groupName) =>
            $"{(groupName.IsNullOrEmpty() ? "Unnamed" : groupName)} Texture Group";
        ...
        private void Save() =>
            EditorDataTextureDatabase.instance.RefreshDatabaseItem(castedTarget, true, true, true);

        private void LoadTexturesFromFolder()
        {
            castedTarget.LoadTexturesFromFolder();
        ...
        private void InitializeEditor()
        {
            ...
            prefixComponentField = FluidField.Get("Auto-Remove Filename Prefix (action executed when setting textures names)");
            TextField prefixTextField = DesignUtils.NewTextField("RemovePrefixFromTextureName", true).SetStyleFlexGrow(1);
        ...
        {
            arrayProperty = serializedObject.FindProperty("Textures");
            ...
            fluidListView.SetListDescription("List of .png textures");
        ...
        public void UpdateSearchResults()
        {
            ...
            for (int i = 0; i < arrayProperty.arraySize; i++)
            {
                ...
                if (!Regex.IsMatch(arrayElementAtIndex.FindPropertyRelative("TextureName").stringValue, $"{fluidListView.searchBox.searchPattern}", RegexOptions.IgnoreCase | RegexOptions.IgnorePatternWhitespace))
                    continue;
