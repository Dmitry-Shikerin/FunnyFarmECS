...
{
    [CustomEditor(typeof(EditorDataSelectableColorPalette))]
    public class EditorDataSelectableColorPaletteEditor : UnityEditor.Editor, ISearchable
    {
        private EditorDataSelectableColorPalette castedTarget => (EditorDataSelectableColorPalette)target;
        ...
        private static string ListViewTitle(string paletteName) =>
            $"{(paletteName.IsNullOrEmpty() ? "Unnamed" : paletteName)} Selectable Color Palette";

        private void SortByName()
        {
            Undo.RecordObject(castedTarget, "Sort");
            castedTarget.SortByColorName();
        }

        private void Save() =>
            EditorDataSelectableColorDatabase.instance.RefreshDatabaseItem(castedTarget, true, true, true);

        private void InitializeEditor()
        {
            root = DesignUtils.GetEditorRoot();

            nameComponentField = FluidField.Get("Selectable Palette Name").SetIcon(EditorSpriteSheets.EditorUI.Components.EditorSelectableColorPalette);
        ...
        {
            arrayProperty = serializedObject.FindProperty("SelectableColors");
            ...
            fluidListView.SetListDescription("List of selectable colors");
            ...
            fluidListView.AddToolbarElement(FluidListView.Buttons.sortAzButton.SetOnClick(SortByName).SetTooltip("Order colors by color name"));
