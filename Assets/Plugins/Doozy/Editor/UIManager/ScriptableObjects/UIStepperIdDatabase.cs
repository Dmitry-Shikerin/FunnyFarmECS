...
{
    [Serializable]
    public class UIStepperIdDatabase : SingletonEditorScriptableObject<UIStepperIdDatabase>, IUpdateCallback
     {
        ...
        [SerializeField] private UIStepperIdDataGroup Database;
        public UIStepperIdDataGroup database => Database ??= new UIStepperIdDataGroup();

        [RestoreData(nameof(UISliderIdDatabase))]
        public static UIStepperIdDatabase RestoreData() =>
            instance;
        
        public UIStepperIdDatabase() => Database = new UIStepperIdDataGroup();

        public (bool, string) CanImportRoamingDatabases(List<UIStepperIdRoamingDatabase> roamingDatabases)
        {
            roamingDatabases ??= new List<UIStepperIdRoamingDatabase>();         //avoid null
        ...
        {
            var roamingDatabases = databases.Cast<UIStepperIdRoamingDatabase>().ToList();
            bool canImport;
            string message;
            ...
            foreach (UIStepperIdRoamingDatabase roamingDatabase in roamingDatabases)
            {
                counter++;
        ...
        {
            string[] guids = AssetDatabase.FindAssets($"t:{nameof(UIStepperIdRoamingDatabase)}");
            if (guids == null || guids.Length == 0)
            {
                EditorUtility.DisplayDialog("Import Databases", $"No {nameof(UIStepperIdRoamingDatabase)} databases were found", "Ok");
                return false;
            }
            ...
            {
                counter++;
                UIStepperIdRoamingDatabase asset = AssetDatabase.LoadAssetAtPath<UIStepperIdRoamingDatabase>(AssetDatabase.GUIDToAssetPath(guid));
