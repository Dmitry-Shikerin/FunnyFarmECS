...
{
    [Serializable]
    public class UITagIdDatabase : SingletonEditorScriptableObject<UITagIdDatabase>, IUpdateCallback
    {
        ...
        {
            roamingDatabases ??= new List<UITagIdRoamingDatabase>();            //avoid null
        ...
        {
            var roamingDatabases = databases.Cast<UITagIdRoamingDatabase>().ToList();
            bool canImport;
            string message;
            ...
            foreach (UITagIdRoamingDatabase roamingDatabase in roamingDatabases)
            {
                counter++;
        ...
        {
            string[] guids = AssetDatabase.FindAssets($"t:{nameof(UITagIdRoamingDatabase)}");
            if (guids == null || guids.Length == 0)
            {
                EditorUtility.DisplayDialog("Import Databases", $"No {nameof(UITagIdRoamingDatabase)} databases were found", "Ok");
                return false;
            }
            ...
            {
                counter++;
                UITagIdRoamingDatabase asset = AssetDatabase.LoadAssetAtPath<UITagIdRoamingDatabase>(AssetDatabase.GUIDToAssetPath(guid));
        ...
        public bool ExportRoamingDatabase(string databaseName, List<string> selectedCategories)
        {
            ...
            UITagIdRoamingDatabase roamingDatabase = CreateInstance<UITagIdRoamingDatabase>();
            roamingDatabase.databaseName = databaseName;
            roamingDatabase.database.ClearDatabase();
            string savePath = EditorUtility.SaveFilePanelInProject("Export Roaming Database", $"{databaseName}_{nameof(UITagIdRoamingDatabase)}", "asset", "Save to");
