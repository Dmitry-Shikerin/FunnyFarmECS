...
{
    [Serializable]
    public class UIToggleIdDatabase : SingletonEditorScriptableObject<UIToggleIdDatabase>, IUpdateCallback
    {
        ...
        public (bool, string) CanImportRoamingDatabases(List<UIToggleIdRoamingDatabase> roamingDatabases)
        {
            roamingDatabases ??= new List<UIToggleIdRoamingDatabase>(); //avoid null
        ...
        {
            var roamingDatabases = databases.Cast<UIToggleIdRoamingDatabase>().ToList();
            bool canImport;
            string message;
            ...
            foreach (UIToggleIdRoamingDatabase roamingDatabase in roamingDatabases)
            {
                counter++;
        ...
        {
            string[] guids = AssetDatabase.FindAssets($"t:{nameof(UIToggleIdRoamingDatabase)}");
            if (guids == null || guids.Length == 0)
            {
                EditorUtility.DisplayDialog("Import Databases", $"No {nameof(UIToggleIdRoamingDatabase)} databases were found", "Ok");
                return false;
            }
            ...
            {
                counter++;
                UIToggleIdRoamingDatabase asset = AssetDatabase.LoadAssetAtPath<UIToggleIdRoamingDatabase>(AssetDatabase.GUIDToAssetPath(guid));
        ...
        public bool ExportRoamingDatabase(string databaseName, List<string> selectedCategories)
        {
            ...
            UIToggleIdRoamingDatabase roamingDatabase = CreateInstance<UIToggleIdRoamingDatabase>();
            roamingDatabase.databaseName = databaseName;
            roamingDatabase.database.ClearDatabase();
            string savePath = EditorUtility.SaveFilePanelInProject("Export Roaming Database", $"{databaseName}_{nameof(UIToggleIdRoamingDatabase)}", "asset", "Save to");
