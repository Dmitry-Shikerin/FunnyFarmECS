...
{
    public class UITagIdExtensionGenerator
    {
        private static string templateName => nameof(UITagIdExtensionGenerator).Replace("Generator", "");
        ...
        public static bool Run(bool saveAssets = true, bool refreshAssetDatabase = false, bool silent = false)
        {
            ...
            UITagIdDataGroup dataGroup = UITagIdDatabase.instance.database;
            if (!UITagIdDatabase.instance.database.isEmpty)
        ...
        private static string InjectContent(string data, Func<IEnumerable<string>> getCategories, Func<string, IEnumerable<string>> getNames)
        {
            ...
            for (int categoryIndex = 0; categoryIndex < categories.Count; categoryIndex++)
            {
                ...
                //ACCESSOR//
                {
                    accessorStringBuilder.AppendLine($"        public static IEnumerable<{nameof(UITag)}> GetTags({nameof(UITagId)}.{category} id) => GetTags(nameof({nameof(UITagId)}.{category}), id.ToString());");
