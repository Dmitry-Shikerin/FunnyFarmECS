...
{
    public static class UISliderIdExtensionGenerator
    {
        private static string templateName => nameof(UISliderIdExtensionGenerator).Replace("Generator", "");
        ...
        public static bool Run(bool saveAssets = true, bool refreshAssetDatabase = false, bool silent = false)
        {
            ...
            UISliderIdDataGroup dataGroup = UISliderIdDatabase.instance.database;
            if (!UISliderIdDatabase.instance.database.isEmpty)
        ...
        private static string InjectContent(string data, Func<IEnumerable<string>> getCategories, Func<string, IEnumerable<string>> getNames)
        {
            ...
            for (int categoryIndex = 0; categoryIndex < categories.Count; categoryIndex++)
            {
                ...
                //ACCESSOR//
                {
                    accessorStringBuilder.AppendLine($"        public static IEnumerable<{nameof(UISlider)}> GetSliders({nameof(UISliderId)}.{category} id) => GetSliders(nameof({nameof(UISliderId)}.{category}), id.ToString());");
                    accessorStringBuilder.AppendLine($"        public static bool SelectSlider({nameof(UISliderId)}.{category} id) => SelectSlider(nameof({nameof(UISliderId)}.{category}), id.ToString());");
