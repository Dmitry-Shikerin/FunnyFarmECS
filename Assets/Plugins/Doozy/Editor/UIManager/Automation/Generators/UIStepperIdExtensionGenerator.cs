...
{
    public class UIStepperIdExtensionGenerator
    {
        private static string templateName => nameof(UIStepperIdExtensionGenerator).Replace("Generator", "");
        ...
        public static bool Run(bool saveAssets = true, bool refreshAssetDatabase = false, bool silent = false)
        {
            ...
            UIStepperIdDataGroup dataGroup = UIStepperIdDatabase.instance.database;
            if (!UIStepperIdDatabase.instance.database.isEmpty)
        ...
        private static string InjectContent(string data, Func<IEnumerable<string>> getCategories, Func<string, IEnumerable<string>> getNames)
        {
            ...
            for (int categoryIndex = 0; categoryIndex < categories.Count; categoryIndex++)
            {
                ...
                //ACCESSOR//
                {
                    accessorStringBuilder.AppendLine($"        public static IEnumerable<{nameof(UIStepper)}> GetSteppers({nameof(UIStepperId)}.{category} id) => GetSteppers(nameof({nameof(UIStepperId)}.{category}), id.ToString());");
