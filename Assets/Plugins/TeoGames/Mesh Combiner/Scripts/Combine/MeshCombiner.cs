...
namespace TeoGames.Mesh_Combiner.Scripts.Combine {
	...
	public class MeshCombiner : AbstractMeshCombiner, IAsyncCombiner {
		...
		private readonly Dictionary<AbstractCombinable, (int cid, BasicMaterial[] materials)>
			_CombinableToMaterial = new Dictionary<AbstractCombinable, (int cid, BasicMaterial[] materials)>();
		...
		private async Task UpdateMesh() {
			...
			foreach (var ren in RendererManagers) {
				...
				await ren.BuildMesh(_TIMER, clearMaterialCache);
			}
