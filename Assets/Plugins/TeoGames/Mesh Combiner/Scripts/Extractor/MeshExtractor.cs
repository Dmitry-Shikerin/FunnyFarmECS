	...
	public class MeshExtractor : MonoBehaviour {
		...
		public virtual async Task Build(Transform original, Vector3 offset, Vector3 rotation, float scale) {
			...
			if (combineMeshes) {
				...
				combiner.rendererTypes = _IsStaticMode ? TargetRendererType.MeshRenderer : rendererTypes;
				combiner.asyncMode = asyncMode;
		...
		private void TriggerUpdate() => onUpdated?.Invoke(combiner.GetRenderers().Select(c =>
