    ...
    public class FileUtils
    {
        ...
        public static void RemakeDirectory(string localFolderPath)
        {
            if (Directory.Exists(localFolderPath))
                DeleteDirectory(localFolderPath, true);
        ...
        public static void CopyTemplateFile(
            string sourceFilePath, string targetFilePath, string srcName, string dstName)
        {
            string parentDirectoryPath = Path.GetDirectoryName(targetFilePath);
            // ReSharper disable once AssignNullToNotNullAttribute
            Directory.CreateDirectory(parentDirectoryPath);
        ...
        /// </summary>
        public static List<string> GetAllFilePathsInFolder(
            string localFolderPath, bool includeHidden = false, bool includeMeta = !IGNORE_META)
        {
        ...
        public static IEnumerable<string> GetFilePathsInFolder(
            string folderPath, bool includeHidden = false, bool includeMeta = !IGNORE_META)
        {
            ...
            if (!includeMeta)
                filePaths = filePaths.Where(path => !IsMetaFile(path));

            // Directory.GetFiles() returns platform dependent delimiter, so make sure replace with "/"
            if (Path.DirectorySeparatorChar != UNITY_FOLDER_SEPARATOR)
            {
                filePaths = filePaths.Select(filePath => filePath.Replace(
                    Path.DirectorySeparatorChar.ToString(), UNITY_FOLDER_SEPARATOR.ToString()));
            }
        ...
        public static string PathCombine(params string[] paths)
        {
            ...
            string[] restPaths = new string[paths.Length - 2];
            Array.Copy(paths, 2, restPaths, 0, restPaths.Length);
            
            foreach (string path in restPaths)
                combinedPath = _PathCombine(combinedPath, path);
        ...
        public static bool IsMetaFile(string filePath) =>
            filePath.EndsWith(UNITY_METAFILE_EXTENSION);
        ...
        {
            string[] directoryPath = Directory.GetDirectories(
                Application.dataPath, directoryName, SearchOption.AllDirectories);
            if (directoryPath.Length == 0)
            {
                if (debug)
                    Debug.LogError("You searched for the [" + directoryName + "] folder, but no folder with that name exists in the current project.");
